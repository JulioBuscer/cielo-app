...
import {
  useActiveFeedingSession, useFeedingHistory, useStartFeeding,
  type FeedingType, type BottleSubtype,
} from '@/src/hooks/useFeedingSessions';
import {
  useActiveSleepSession, useStartSleep,
...
import {
  TimelineBubble, FeedingSessionBubble, DateSeparator,
} from '@/src/components/ui/TimelineBubbles';

// ─── Botón circular de acción rápida ─────────────────────────────────────────

function QuickBtn({
  emoji, label, onPress, disabled, loading, bgColor, size = 52,
}: {
  emoji: string; label: string; onPress: () => void;
  disabled?: boolean; loading?: boolean; bgColor: string; size?: number;
}) {
  return (
    <TouchableOpacity
    ...
    >
      <View style={{
        ...
        shadowColor: bgColor, shadowOffset: { width: 0, height: 3 },
      ...
      <Text style={{ fontSize: 9, fontWeight: '800', color: '#9B7A88', textAlign: 'center', lineHeight: 12 }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
...
function EventPickerModal({ visible, onClose, onSelect }: {
  ...
  return (
    <View style={{
      ...
      backgroundColor: 'rgba(45,27,38,0.4)', justifyContent: 'flex-end', zIndex: 100,
    }}>
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          {available.map(t => (
            <TouchableOpacity
              key={t.id} onPress={() => onSelect(t.id)}
              style={{ backgroundColor: '#FFF0F5', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 6 }}
            >
...
export default function TimelineScreen() {
  ...
  const handleStartFeeding = async (type: FeedingType, bottleSubtype?: BottleSubtype) => {
    if (!baby) return;
    setLoadingType(type);
    try { await startFeeding.mutateAsync({ babyId: baby.id, type, bottleSubtype }); }
  ...
  const handleToggleSleep = async () => {
    ...
    try { await startSleep.mutateAsync({ babyId: baby.id }); }
  ...
  const handleSendNote = () => {
    ...
    saveEvent.mutate({
      babyId: baby.id, eventTypeId: 'note',
      notes: note.trim(),
  ...
  const handleEventSelect = (typeId: string) => {
    ...
    else router.push({ pathname: '/logs/event/new', params: { preselect: typeId } });
  };
  ...
  const renderItem = ({ item }: { item: TLItem }) => {
    ...
    const isOwn = item.data.profileId === profile?.id;
    const { emoji, label } = TYPE_MAP[item.data.eventTypeId] ?? { emoji: '📝', label: item.data.eventTypeId };
    return (
      <TimelineBubble
        event={item.data} eventTypeEmoji={emoji} eventTypeLabel={label}
        isOwn={isOwn} profileName={!isOwn ? 'Otro cuidador' : undefined}
      />
    );
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FF8AB3' }} edges={['top']}>
      ...
      {/* ── Header WhatsApp ── */}
      <View style={{
        backgroundColor: '#FF8AB3', paddingHorizontal: 16, paddingVertical: 10,
        flexDirection: 'row', alignItems: 'center', gap: 10,
        shadowColor: '#FF5C9A', shadowOffset: { width: 0, height: 2 },
        ...
        <TouchableOpacity style={{ flex: 1 }} onPress={() => router.push('/baby/profile')}>
        ...
        <TouchableOpacity
          ...
          style={{ backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 99, paddingHorizontal: 10, paddingVertical: 5 }}
        >
      ...
      >
        <View style={{ flex: 1, backgroundColor: '#FFF0F5' }}>
          ...
          <View style={{
            ...
            {/* Fila de botones */}
            <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', paddingHorizontal: 2, marginBottom: 10 }}>
      ...
      <BottleSubtypeModal
        ...
        onSelect={(subtype) => { setShowBottleModal(false); handleStartFeeding('bottle', subtype); }}
      />
      <EventPickerModal
