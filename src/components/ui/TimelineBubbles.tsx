...
import { FEEDING_LABELS, BOTTLE_SUBTYPE_LABELS } from '@/src/hooks/useFeedingSessions';
import type { TimelineEvent, FeedingSession } from '@/src/db/schema';
...
function MetaTag({ label, color = '#FF5C9A', bg = '#FFF0F5' }: {
...
export function TimelineBubble({
  event, eventTypeEmoji, eventTypeLabel,
  isOwn, profileName, onPress,
}: {
  event: TimelineEvent;
    ...
    >
      <View style={{
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
          ...
          <Text style={{ fontWeight: '900', fontSize: 14, color: '#2D1B26' }}>{eventTypeLabel}</Text>
        </View>
              ...
              <View style={{ flexDirection: 'row', gap: 3 }}>
                {[1,2,3,4,5].map(n => (
                  <View key={n} style={{
                    ...
                    backgroundColor: n <= (meta.peeIntensity ?? 0) ? '#F5C842' : '#FFE4EE',
                  }} />
                ))}
                <Text style={{ fontSize: 11, color: '#9B7A88', fontWeight: '700', marginLeft: 4 }}>💧</Text>
              </View>
              <View style={{ flexDirection: 'row', gap: 3 }}>
                {[1,2,3,4,5].map(n => (
                  <View key={n} style={{
                    width: 16, height: 16, borderRadius: 99,
                    backgroundColor: n <= (meta.poopIntensity ?? 0) ? '#8B5E3C' : '#FFE4EE',
                  }} />
                ))}
                <Text style={{ fontSize: 11, color: '#9B7A88', fontWeight: '700', marginLeft: 4 }}>💩</Text>
              </View>
        ...
        {event.eventTypeId === 'medication' && meta?.medicineName && (
          <MetaTag label={`${meta.medicineName}${meta.dose ? ` · ${meta.dose}` : ''}`} color="#7C3AED" bg="#F5F3FF" />
        )}
        ...
        {(event.eventTypeId === 'weight' || event.eventTypeId === 'height') && meta && (
          <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {meta.weightGrams && <MetaTag label={`${(meta.weightGrams/1000).toFixed(2)} kg`} color="#0EA5E9" bg="#F0F9FF" />}
            {meta.heightMm    && <MetaTag label={`${(meta.heightMm/10).toFixed(1)} cm`}     color="#0EA5E9" bg="#F0F9FF" />}
          </View>
        )}

        {/* Notas */}
        {event.notes && (
          <Text style={{ fontSize: 13, color: '#2D1B26', marginTop: 4, lineHeight: 18 }}>
            {event.notes}
          </Text>
        )}
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 4, marginTop: 4 }}>
          {profileName && (
            <Text style={{ fontSize: 10, fontWeight: '800', color: '#FF5C9A' }}>{profileName}</Text>
          )}
    ...
    >
      <View style={{
        ...
        {session.durationSec != null && (
          <View style={{
            backgroundColor: '#FF5C9A', borderRadius: 99,
        ...
        {session.notes && (
          <Text style={{ fontSize: 13, color: '#2D1B26', marginTop: 2 }}>{session.notes}</Text>
        )}
  ...
  return (
    <View style={{ alignItems: 'center', marginVertical: 12 }}>
      <View style={{
        ...
        borderRadius: 99, paddingHorizontal: 14, paddingVertical: 4,
      }}>
        <Text style={{ fontSize: 12, fontWeight: '700', color: '#9B7A88' }}>{label}</Text>
      </View>
    </View>
  );
}
  ...
  return (
    <View style={{ alignItems: 'center', marginVertical: 3 }}>
      <View style={{
        ...
        borderRadius: 99, paddingHorizontal: 12, paddingVertical: 3,
      }}>
        <Text style={{ fontSize: 11, fontWeight: '700', color: '#9B7A88' }}>{label}</Text>
      </View>
    </View>
  );
}
